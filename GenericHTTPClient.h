...
public:
	struct GenericHTTPArgument
    {
        ...
        public:
            GenericHTTPArgument(int iBufferSize)
            {
                szValue = new TCHAR[iBufferSize];

                memset(szName, 0, __SIZE_HTTP_ARGUMENT_NAME);
                memset(szValue, 0, iBufferSize);
            ...
            TCHAR	*szValue;
            DWORD	dwType;
            operator==(const GenericHTTPArgument &argV){
