...
{
    m_iHttpBufferSize = 0;
    _szHTTPResponseHTML  = NULL;
    _szHTTPResponseHeader  = NULL;
...
GenericHTTPClient::~GenericHTTPClient()
{
    ...
    delete[] _szHTTPResponseHTML;
    _szHTTPResponseHTML = NULL;

    delete[] _szHTTPResponseHeader;
    _szHTTPResponseHeader = NULL;

    ClearArgs();
}

void GenericHTTPClient::GrowBuffers(int p_iNewSize)
{
    if (p_iNewSize > m_iHttpBufferSize)
    {
        m_iHttpBufferSize = p_iNewSize;
        if (_szHTTPResponseHTML)
        {
            delete[] _szHTTPResponseHTML;
            _szHTTPResponseHTML = NULL;
        }
        if (_szHTTPResponseHeader)
        {
            delete[] _szHTTPResponseHeader;
            _szHTTPResponseHeader = NULL;
        }
        _szHTTPResponseHTML = new char[m_iHttpBufferSize];
        _szHTTPResponseHeader = new char[m_iHttpBufferSize];
    }
}
...
#ifdef  _DEBUG
        LPVOID  lpMsgBuffer;
        DWORD   dwRet   = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, ::GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<LPTSTR>(&lpMsgBuffer), 0, NULL);
...
BOOL GenericHTTPClient::Request(LPCTSTR szURL, int nMethod, LPCTSTR szAgent)
{
    ...
    if (Connect(szAddress, szAgent, dwPort))
    {
        ...
        {
            if (!Response((PBYTE) _szHTTPResponseHeader, m_iHttpBufferSize, (PBYTE) _szHTTPResponseHTML, m_iHttpBufferSize, dwSize))
            {
                bReturn = FALSE;
            }
        }
...
BOOL GenericHTTPClient::RequestGet(LPCTSTR szURI)
{
    ...
    _hHTTPRequest = ::HttpOpenRequest(_hHTTPConnection, __HTTP_VERB_GET, // HTTP Verb
                                                            szURI, // Object Name
                                                            HTTP_VERSION, // Version
                                                            "", // Reference
 & szAcceptType, // Accept Type
                                                            INTERNET_FLAG_KEEP_CONNECTION | INTERNET_FLAG_NO_CACHE_WRITE, 0); // context call-back point
...
BOOL GenericHTTPClient::RequestPost(LPCTSTR szURI)
{
    ...
    _hHTTPRequest = ::HttpOpenRequest(_hHTTPConnection, __HTTP_VERB_POST, // HTTP Verb
                                                            szURI, // Object Name
                                                            HTTP_VERSION, // Version
                                                            "", // Reference
 & szAcceptType, // Accept Type
                                                            INTERNET_FLAG_KEEP_CONNECTION | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_FORMS_SUBMIT, 0); // context call-back point
...
/*
#ifdef  _DEBUG
    TCHAR   szHTTPRequest[m_iHttpBufferSize]   = "";
    DWORD   dwHTTPRequestLength                 = m_iHttpBufferSize;

    ::ZeroMemory(szHTTPRequest, m_iHttpBufferSize);
...
*/
    ...
    INTERNET_BUFFERS    InternetBufferIn    =
    {
        0
    };
...
DWORD GenericHTTPClient::ResponseOfBytes(PBYTE pBuffer, DWORD dwSize)
{
    ...
    if (!::InternetReadFile(_hHTTPRequest, pBuffer, dwSize, &dwBytes))
    {
        _dwError = ::GetLastError();
#ifdef  _DEBUG
        LPVOID  lpMsgBuffer;
...
{
    GenericHTTPArgument* arg = new GenericHTTPArgument(m_iHttpBufferSize);
...
void GenericHTTPClient::ClearArgs()
{
    GenericHTTPArgument* pArg = NULL;
    while (_vArguments.size() > 0)
    {
        pArg = _vArguments.back();
        _vArguments.pop_back();
        delete pArg;
    }
...
DWORD GenericHTTPClient::AllocMultiPartsFormData(PBYTE& pInBuffer, LPCTSTR szBoundary)
{
    ...
    for (itArgv = _vArguments.begin(); itArgv < _vArguments.end(); ++itArgv)
    {
        ...
        case    GenericHTTPClient::TypeNormal:
            pBuffer = (PBYTE) ::LocalAlloc(LPTR, m_iHttpBufferSize * 2);

            _stprintf((TCHAR *) pBuffer, "--%s\r\n"
                            "Content-Disposition: form-data; name=\"%s\"\r\n"
                            "\r\n"
                            "%s\r\n", szBoundary, (*itArgv)->szName, (*itArgv)->szValue);
        ...
        case    GenericHTTPClient::TypeBinary:
            ...
            HANDLE  hFile               = ::CreateFile((*itArgv)->szValue, GENERIC_READ, // desired access
            ...
            _stprintf((TCHAR *) pBuffer, "--%s\r\n"
                            ...
                            "Content-Type: %s\r\n"
                            "\r\n", szBoundary, (*itArgv)->szName, (*itArgv)->szValue, GetContentType((*itArgv)->szValue));
